...
export class ProductOrderQueries {
    ...
    async create(productId:string, orderId:string, quantity:number=1): Promise<ProductOrder[]> {
        try {
            const connection = await client.connect();
            const sql = 'INSERT INTO productOrders (productId, orderId, quantity) VALUES ($1, $2, $3)';
